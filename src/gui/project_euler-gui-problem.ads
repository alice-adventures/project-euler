...
package Project_Euler.GUI.Problem is
   ...
   type GUI_Problem_Type is
     abstract new Status_Type and CLI_Problem_Type with private;

   type Pointer_To_GUI_Problem_Class is access all GUI_Problem_Type'Class;

   --  -----------------------------------------------
   --  Subprograms implemented by the Problem/GUI_Problem_Type

   procedure Plotter_Setup
     (Problem : GUI_Problem_Type;
      Plotter : Pointer_To_Plotter_Class) is abstract;
   --  Prepares the Plotter for a graphical representation of a Problem.

   procedure On_Start
     (Problem : in out GUI_Problem_Type;
      Plotter :        Pointer_To_Plotter_Class) is abstract;
   --  Main procedure to be implemented by Problems inheriting from
   --  GUI_Problem_Type. Runners will call it when the user press the Start button.

   --  ---------------------------------------------------
   --  Common subprograms used by all implemented Problems

   procedure Start (Problem : in out GUI_Problem_Type);
   ...
   procedure Stop (Problem : in out GUI_Problem_Type);
   ...
   procedure Pause (Problem : in out GUI_Problem_Type);
   ...
   function Is_Started (Problem : GUI_Problem_Type) return Boolean;
   --  Return True if the problem has already been started.

   function Is_Stopped (Problem : GUI_Problem_Type) return Boolean;
   --  Return True if the problem has been stopped.

   procedure Wait_To_Continue (Problem : in out GUI_Problem_Type);
   ...
   procedure On_Step (Problem : in out GUI_Problem_Type) renames Pause;
   --  Allows the Runner to set the status of the problem to Paused.

   procedure On_Continue (Problem : in out GUI_Problem_Type);
   --  Allows the Runner to set the status of the problem to Run again.

   procedure On_Stop (Problem : in out GUI_Problem_Type) renames Stop;
...
private
   ...
   type GUI_Problem_Type is
   abstract new Status_Type and CLI_Problem_Type with record
